...
void EnemyHeilcoptor::update(float delta_time)
{
	...
	ImGui::DragFloat3("forward", transform_.forward());
	int current_waypoint = get_way_point_num();
	...
	float pitch = MathUtility::get_pitch(this, get_player());
	ImGui::DragFloat("Pitch", &pitch);
	float yaw = MathUtility::get_yaw(this, get_player());

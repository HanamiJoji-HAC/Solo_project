...
void StateMachine::change_state(int next_state) {
	if (current_state_num_ == next_state) return;

	std::shared_ptr<IState> state = find_state(next_state);
	if (state == nullptr) return;
	current_state_->on_exit();
	previous_state_ = current_state_;
	current_state_ = state;
	current_state_->on_enter();
	previous_state_num_ = current_state_num_;
	current_state_num_ = next_state;
};
...
// 前回のステートを参照する
int StateMachine::get_previous_state() {
	return previous_state_num_;
}